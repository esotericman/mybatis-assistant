...
import org.flmelody.mybatis.dom.MapperBacktrackingUtils;
import org.flmelody.mybatis.util.MybatisConstants;
...
/*        EntityMappingResolverFactory entityMappingResolverFactory
    ...
    public Optional<DbElement> findColumns(DasTable dasTable) {
        ...
        if (child != null) {
            Optional<DbDataSource> first = dbPsiFacade.getDataSources().stream().findFirst();
            if (first.isPresent()){
                return Optional.of(first.get().findElement(child));
            }
        }
        return Optional.empty();
    }
